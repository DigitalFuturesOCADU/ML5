...
function draw() {
    ...
    // Draw hand numbers at the top of the screen
    if (hands.length > 0) {
        hands.forEach((hand, index) => {
            if (hand.keypoints && hand.keypoints.length > 0) {
                // Calculate average X position of hand
                ...
                textSize(20);
                text(`Hand ${index}`, avgX, 20);
            }
        });
        
        // Get and display points based on selection variables
        let point1 = getKeypoint(handPointIndex1, handNumberIndex1);
        let point2 = getKeypoint(handPointIndex2, handNumberIndex2);
        
        if (point1 && point2) {
            // Show the points in different colors
            showPoint(point1, color(255, 0, 0));  // Red
            showPoint(point2, color(0, 0, 255));  // Blue
            
            // Measure and display distance
            let distance = measureDistance(point1, point2);
            
            // Measure and display angle (using point1 as base point)
            let angle = measureAngle(point1, point2);
        }
    }
}
