...
// Define which points to measure between 
let bodyPointIndex1 = 5;  // left shoulder
let bodyPointIndex2 = 6;  // right shoulder
...
function draw() {
    ...
    // If we have a pose, measure between the specified points
    if (poses.length > 0) {
        let bodyPoint1 = getKeypoint(bodyPointIndex1, 0);
        let bodyPoint2 = getKeypoint(bodyPointIndex2, 0);
        
        if (bodyPoint1 && bodyPoint2 && 
            bodyPoint1.confidence > confidenceThreshold && 
            bodyPoint2.confidence > confidenceThreshold) {
            
            // Show the points in different colors
            showPoint(bodyPoint1, color(255, 0, 0));  // Red
            showPoint(bodyPoint2, color(0, 0, 255));  // Blue
            
            // Measure and display distance
            let distance = measureDistance(bodyPoint1, bodyPoint2);
            
            // Measure and display angle (using bodyPoint1 as base point)
            let angle = measureAngle(bodyPoint1, bodyPoint2);
        }
    }
}
